...
import { SummaryIcon, SparklesIcon, CopyIcon, DownloadIcon, CheckIcon } from "@/lib/icons"
import type { SummarizerSession } from "@/lib/types"
import { toast } from "sonner"
import { generateId, cn } from "@/lib/utils"

export default function DocumentSummarizerPage() {
  const [sessions, setSessions] = useState<SummarizerSession[]>(mockSummarizerSessions)
  const [activeSession, setActiveSession] = useState<SummarizerSession | null>(null)
  ...
  const handleSummarize = async () => {
    ...
    toast.info("AI is analyzing and summarizing your document...")
    ...
    const mockSummary = {
      ...
      parties: ["TechCorp Solutions Inc. (Service Provider)", "GlobalTrade Enterprises Ltd. (Client)"],
...
## Key Points
${summary.keyPoints.map((section: any) => `### ${section.category}\n${section.points.map((p: string) => `- ${p}`).join("\n")}`).join("\n\n")}
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-border bg-background px-4 py-3 lg:px-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Transform lengthy documents into concise summaries</p>
          </div>
          ...
          {!summary ? (
            <div className="mx-auto max-w-2xl">
              <div className="mb-8 text-center">
                ...
                <h2 className="mb-2 text-xl font-semibold text-foreground">Upload Document to Summarize</h2>
                <p className="text-muted-foreground">
                  Upload a legal document and our AI will generate a comprehensive summary with key points, risks, and
                  recommendations.
                </p>
              </div>

              <FileUpload maxFiles={1} onFilesChange={handleFilesChange} className="mb-6" />
            ...
            <div className="mx-auto max-w-4xl">
              ...
              <div className="mb-6 rounded-xl border border-border bg-card p-4 lg:p-6">
                <h3 className="mb-4 text-lg font-semibold text-foreground">Document Overview</h3>
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <p className="text-sm text-muted-foreground">Document Type</p>
                    <p className="font-medium text-foreground">{summary.documentType}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Effective Date</p>
                    <p className="font-medium text-foreground">{summary.effectiveDate}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Term Length</p>
                    <p className="font-medium text-foreground">{summary.termLength}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Parties</p>
                    <p className="font-medium text-foreground">{summary.parties.length} parties involved</p>
                  </div>
                </div>
                <div className="mt-4">
              ...
              <div className="mb-6 grid gap-4 sm:grid-cols-2">
                {summary.keyPoints.map((section: any, index: number) => (
                  <div key={index} className="rounded-xl border border-border bg-card p-4 lg:p-6">
                    <h4 className="mb-3 font-semibold text-foreground">{section.category}</h4>
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 lg:p-6">
                  <h4 className="mb-3 font-semibold text-foreground">Identified Risks</h4>
                  <ul className="space-y-2">
                ...
                <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-4 lg:p-6">
                  <h4 className="mb-3 font-semibold text-foreground">Recommendations</h4>
                  <ul className="space-y-2">
